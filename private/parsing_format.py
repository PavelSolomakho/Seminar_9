...
def parse_all_data_between_delimeter(data: str, delimeter_start: str, delimeter_end: str):
    ...
    res = re.findall(delimeter_start + '(.*?)' +
                     delimeter_end, data, re.DOTALL)
    if res is None:
        return []
...
def parse_user_data_by_delimeter(users_data, user_data_delimeter, user_properties_delimeter):
    parsed_users_data = []
    parsed_users = parse_all_data_between_delimeter(
        users_data, user_data_delimeter, user_data_delimeter)
    for user in parsed_users:
        parsed_users_data.append(parse_user_properties_by_delimeter(
            user, user_properties_delimeter))
...
def parse_format_data_by_delimeter(format_data: str, format_id: int):
    ...
    tmp_parsed_format_data = parse_all_data_between_delimeter(
        format_data, formatDataDelimeter, formatDataDelimeter)
    for data in tmp_parsed_format_data:
        parsed_format_data.append(parse_user_data_by_delimeter(
            data, FORMATS[format_id]['userDataDelimeter'], FORMATS[format_id]['userPropertiesDelimeter']))
...
def parse_all_format_data(format_data: str):
    ...
    for format_id in range(len(FORMATS)):
        parsed_format_data.append(
            parse_format_data_by_delimeter(format_data, format_id))
    return parsed_format_data
